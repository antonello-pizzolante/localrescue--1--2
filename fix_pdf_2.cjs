...
const tStart = content.indexOf('  // Damages summary');
const tEnd = content.indexOf('  // Notes', tStart);
...
content = content.substring(0, tStart) + damageNew + content.substring(tEnd);

// Remove the old appended image stuff if there
const rmStart = content.indexOf('  // --- CARROZZERIA IMAGE (Appended at the end or on a new page) ---');
if (rmStart !== -1) {
  const rmEnd = content.indexOf('  return doc;', rmStart);
  if (rmEnd !== -1) {
    content = content.substring(0, rmStart) + content.substring(rmEnd);
  }
}
