...
export function SanitaserviceLogo({ className = '' }: { className?: string }) {
  ...
  const [imageSrc, setImageSrc] = useState(localStorage.getItem('sanitaservice_logo_custom') || null);
...
export function TarantoSoccorsoLogo({ className = '' }: { className?: string }) {
  ...
  const [imageSrc, setImageSrc] = useState(localStorage.getItem('taranto_soccorso_logo_custom') || null);
  const fileInputRef = useRef(null);
