const fs = require('fs');
let content = fs.readFileSync('src/components/Logos.tsx', 'utf8');
...
export function SanitaserviceLogo({ className = '' }: { className?: string }) {
  ...
  return (
    <div className={\`flex items-center justify-center p-1.5 bg-white rounded-lg border border-slate-200/80 shadow-sm relative \${heightClass} \${shrinkClass} \${className}\`}>
      <img src="/sanitaservice-logo.svg" alt="Sanitaservice Logo" className="h-full w-auto object-contain" />
    </div>
  );
}
...
export function TarantoSoccorsoLogo({ className = '' }: { className?: string }) {
  ...
  return (
    <div className={\`flex items-center justify-center p-1.5 bg-white rounded-lg border border-slate-200/80 shadow-sm relative \${heightClass} \${shrinkClass} \${className}\`}>
      <img src="/taranto-soccorso-logo.svg" alt="118 Taranto Soccorso Logo" className="h-full w-auto object-contain" />
    </div>
  );
}
`;
