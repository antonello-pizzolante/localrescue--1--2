...
export function SanitaserviceLogo({ className = '' }: { className?: string }) {
  ...
  useEffect(() => {
    const fetchLogo = async () => {
      try {
        const docRef = doc(db, 'settings', 'sanitaservice_logo');
        const docSnap = await getDoc(docRef);
        if (docSnap.exists() && docSnap.data().imageSrc) {
          setImageSrc(docSnap.data().imageSrc);
          localStorage.setItem('sanitaservice_logo_custom', docSnap.data().imageSrc);
        }
      } catch (err) {
        console.error("Error loading logo from cloud:", err);
      }
    };
    fetchLogo();
  }, []);
...
export function TarantoSoccorsoLogo({ className = '' }: { className?: string }) {
  ...
  useEffect(() => {
    const fetchLogo = async () => {
      try {
        const docRef = doc(db, 'settings', 'taranto_soccorso_logo');
        const docSnap = await getDoc(docRef);
        if (docSnap.exists() && docSnap.data().imageSrc) {
          setImageSrc(docSnap.data().imageSrc);
          localStorage.setItem('taranto_soccorso_logo_custom', docSnap.data().imageSrc);
        }
      } catch (err) {
        console.error("Error loading logo from cloud:", err);
      }
